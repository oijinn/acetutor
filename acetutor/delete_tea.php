...
<title>Delete Staff - ACE Tutor</title>
...
<?php
	...
	$fid = $_SESSION['id'];
	$result = mysqli_query($con, "DELETE FROM staff WHERE fid='$fid'");

	if (!$result){
		echo "<script type=\"text/javascript\">";
		echo "Swal.fire({";
		echo "title: 'Staff is not deleted!',";
		...
		echo "<script type=\"text/javascript\">";
		echo "Swal.fire({";
		echo "title: 'Staff is deleted successfully!',";
	    echo "icon: 'success'}).";
	    echo "then((result) => {";
	  	echo "if (result.isConfirmed) {";
		echo "window.location.href='InfoTea.php';}})";
